...
function toCsvCell(value: string | number): string {
  ...
  const text = /^[=+\-@\t\r]/.test(rawText.trimStart()) ? `'${rawText}` : rawText;
  if (/[",\n\r]/.test(text)) {
...
export default function InventoryCsvCheckerApp() {
  ...
  function onFile(
    ...
    if (file.size > MAX_FILE_BYTES) {
      setInputNotice("This CSV is too large for an in-browser check. Please use a file under 2 MB.");
      event.target.value = "";
      return;
    }
    ...
    reader.onload = () => {
      ...
      if (nextText.length > MAX_TEXT_CHARS) {
        setter(nextText.slice(0, MAX_TEXT_CHARS));
        setInputNotice("Only the first 800,000 characters were loaded for this browser check.");
        return;
      }
      setter(nextText);
      trackEvent(eventName, {
  ...
  function updateCsvText(value: string, setter: (nextValue: string) => void) {
    if (value.length > MAX_TEXT_CHARS) {
      setter(value.slice(0, MAX_TEXT_CHARS));
      setInputNotice("Only the first 800,000 characters can be checked at once.");
      return;
    }
    setInputNotice("");
    setter(value);
      ...
      <section className="hero" id="checker">
        ...
        <section className="toolShell" aria-label="Shopify inventory CSV checker">
          <div className="csvPanel">
            ...
            <textarea value={oldText} onChange={(event) => updateCsvText(event.target.value, setOldText)} spellCheck={false} aria-label="Original Shopify inventory CSV" />
          </div>
          ...
          <div className="csvPanel">
            ...
            <textarea value={newText} onChange={(event) => updateCsvText(event.target.value, setNewText)} spellCheck={false} aria-label="Edited Shopify inventory CSV" />
          </div>
        </section>

        {inputNotice ? (
          <p className="inputNotice" role="alert">
            {inputNotice}
          </p>
        ) : null}
